...
if( !function_exists('wpestate_property_similar_listings_v2') ):
    function wpestate_property_similar_listings_v2($postID,$is_tab='',$tab_active_class=''){
        ...
        if($is_tab=='yes'){
            $label = esc_html('Similar Listings','wpresidencce');
...
if( !function_exists('wpestate_property_reviews_v2') ):
    function wpestate_property_reviews_v2($postID,$is_tab='',$tab_active_class=''){
        ...
        include( locate_template ('/templates/listing_templates/property-page-templates/property_reviews_v2.php' ) );
        $content=ob_get_contents();
        ob_end_clean();
...
if( !function_exists('wpestate_property_agent_area_v2') ):
    function wpestate_property_agent_area_v2($postID,$wpestate_options='',$is_tab='',$tab_active_class=''){
        ...
        if($is_tab=='yes'){
            $label = esc_html('Agent','wpresidence');
            return wpestate_property_page_create_tab_item($content,$label,$data['tab_id'],$tab_active_class);
        }else{
            print trim($content);
        }
...
function wpestate_property_page_create_tab_item($content,$label,$tab_id,$class=''){
    $return_array=array();
    // Begin Alan Hong change language with tab panel
    $we_lang = ICL_LANGUAGE_CODE;
    switch ($label) {
        case 'Mô tả':
            $label = ($we_lang == 'en') ? 'Description' : $label;
            break;

        case 'Thông tin mô tả':
            $label = ($we_lang == 'en') ? 'Description' : $label;
            break;

        case 'Địa điểm':
            $label = ($we_lang == 'en') ? 'Address' : $label;
            break;

        case 'Chi tiết':
            $label = ($we_lang == 'en') ? 'Details' : $label;
            break;
        ...
        case 'Đánh giá':
            $label = ($we_lang == 'en') ? 'Reviews' : $label;
            break;
    ...
    // End Alan Hong change language with tab panel
...
/**
* We metrics customize add form contact follow the property
* Created by Alan hong
*
*/
if( !function_exists('wpestate_property_we_contact_form') ):
    function wpestate_property_we_contact_form($postID, $is_tab='', $tab_active_class=''){
        $id = $postID;      
        $we_lang = ICL_LANGUAGE_CODE;
        $label = 'Liên hệ thuê phòng';
        switch ($we_lang) {
            case 'en':
        ...
        $acc_id = 'wpestate_property_we_fjo22d';


        ob_start();
        include( locate_template ('/templates/listing_templates/property-page-templates/property_contact.php') );
        $content=ob_get_contents();
        ob_end_clean();
...
function estate_listing_address($post_id,$wpestate_prop_all_details='',$columns=''){
    ...
    if ($property_address != ''){
        $return_string.='<div class="listing_detail col-md-'.$colmd.'"><strong>'.esc_html__($addressLb,'wpresidence').':</strong> ' . $property_address . '</div>';
    }
    if ($property_city != ''){
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.esc_html__($wardLb,'wpresidence').':</strong> ' .$property_city. '</div>';
    }
    if ($property_area != ''){
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.esc_html__($buildingLb,'wpresidence').':</strong> ' .$property_area. '</div>';
    }
    if ($property_county != ''){
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.esc_html__($districtLb,'wpresidence').':</strong> ' . $property_county . '</div>';
    }

    // Begin Alan Hong Disable Zip and Country
    // if ($property_zip != ''){
    ...
    // }
    // End Alan Hong Disable Zip and Country
...
function estate_listing_details($post_id,$wpestate_prop_all_details='',$columns=''){
    ...
    // Biến điều khiển ngôn ngữ sử dụng
    $we_lang = ICL_LANGUAGE_CODE;
    $return_string='';
    /*
    * Chi tiết
    */
    $we_detail_lb = 'Chi tiết';
    $we_prop_room_id = 'Mã phòng';
    $we_prop_price = 'Giá';
    $we_prop_size = 'Diện tích';
    switch ($we_lang) {
        case 'en':
            // Dịch label
            $we_detail_lb = str_replace('Chi tiết', 'Details', $we_detail_lb);
            $we_prop_room_id = str_replace('Mã phòng', 'Room id', $we_prop_room_id);
            $we_prop_price = str_replace('Giá', 'Price', $we_prop_price);
            $we_prop_size = str_replace('Diện tích', 'Size', $we_prop_size);
            break;
        
        default:
            break;
        }
    $return_string.= '<div class="listing_detail col-md-12"><strong><h4 style="color: #FF7925!important;">'.$we_detail_lb.'</h4></strong></div>';

    // Mã phòng we_prop_code
    $we_prop_code = get_post_meta($post_id, 'we_prop_code', true) ;
    $we_prop_code_val = 'Liên hệ';
    $we_prop_code_lb = 'Mã phòng';
        if($we_prop_code!=''){
            $we_prop_code_val = strval($we_prop_code);
        }
        
        switch ($we_lang) {
            case 'en':
                // Dịch label
                $we_prop_code_lb = str_replace('Mã phòng', 'Room id', $we_prop_code_lb);
                break;
    ...
    $to_style = 'style="color:#fff; background:#39b54a; display: inline-block; padding: 0px 10px; border-radius: 3px;"';
    $we_prop_code_html = '<div '.$to_style.'>' . $we_prop_code_val . '</div>';
    $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_code_lb).':</strong> ' .$we_prop_code_html. '</div>';  

    // $return_string.='<div class="listing_detail col-md-'.$colmd.'" id="propertyid_display"><strong>'.esc_html__($we_prop_room_id,'wpresidence'). ':</strong> '.$post_id.'</div>';

    if ($price !='' ){
    switch ($we_lang) {
        case 'en':
            $price = str_replace('month', 'month', $price);
            break;
        
        default:
            $price = str_replace('month', 'tháng', $price);
            break;
        }
        $return_string.='<div class="listing_detail col-md-'.$colmd.' property_default_price"><strong>'.esc_html__($we_prop_price,'wpresidence'). ':</strong> '. $price.'</div>';
    }
    ...
    if ($property_size != ''){
        $return_string.= '<div class="listing_detail col-md-'.$colmd.' property_default_property_size"><strong>'.esc_html__($we_prop_size,'wpresidence').':</strong> ' . $property_size . '</div>';
    }
    ...
    //     $return_string.= '<div class="listing_detail col-md-'.$colmd.' property_default_rooms"><strong>'.esc_html__('Số phòng','wpresidence').':</strong> ' . $property_rooms . '</div>';
    // }

    $return_string .= '<hr style="color: #003882; border: 0.25px solid;">';

    // Begin Alan Hong show custmo fields
    /*
    * Tình trạng
    */
    $we_status_lb = 'Tình trạng';
    switch ($we_lang) {
        case 'en':
            // Dịch label
            $we_status_lb = str_replace('Tình trạng', 'Status', $we_status_lb);
            break;
        
        default:
            break;
        }
    $return_string.= '<div class="listing_detail col-md-12"><strong><h4 style="color: #FF7925!important;">'.$we_status_lb.'</h4></strong></div>';
    // Thú cưng we_prop_pet
    $we_prop_pet = get_post_meta($post_id, 'we_prop_pet', true) ;
    $we_prop_pet_val = 'Liên hệ';
    $we_prop_pet_lb = 'Nuôi pet';
        if($we_prop_pet!=''){
            $we_prop_pet_val = strval($we_prop_pet);
        }
        
        switch ($we_lang) {
            case 'en':
                // Dịch giá trị
                $we_prop_pet_val = str_replace('Có', 'Yes', $we_prop_pet_val);
                $we_prop_pet_val = str_replace('Không', 'Not', $we_prop_pet_val);
                $we_prop_pet_val = str_replace('Liên hệ', 'Call', $we_prop_pet_val);
                // Dịch label
                $we_prop_pet_lb = str_replace('Nuôi pet', 'Pet', $we_prop_pet_lb);
                break;
    ...
    $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_pet_lb).':</strong> ' .$we_prop_pet_val. '</div>';

    // Số người we_prop_people
    $we_prop_people = get_post_meta($post_id, 'we_prop_people', true) ;
    $we_prop_people_val = '0';
    $we_prop_people_lb = 'Số người';
        if($we_prop_people!=''){
            $we_prop_people_val = strval($we_prop_people);
        }

        switch ($we_lang) {
            case 'en':
                // Dịch label
                $we_prop_people_lb = str_replace('Số người', 'People', $we_prop_people_lb);
                break;
    ...
    $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_people_lb).':</strong> ' .$we_prop_people_val. '</div>';

    // Trạng thái phòng
    $we_prop_status = get_post_meta($post_id, 'we_prop_status', true) ;
    $we_prop_status_val = 'Liên hệ';
    $we_prop_status_lb = 'Trạng thái phòng';
        if($we_prop_status!=''){
            $we_prop_status_val = strval($we_prop_status);
        }
        $to_background = '#003882d9';
        switch ($we_prop_status_val) {
            case 'Đã thuê':
                $to_background = '#ff6900';
                break;

            case 'Còn phòng':
                $to_background = '#39b54a';
                break;

            default:
                break;
        }
        $to_style = 'style="color:#fff; background: '.$to_background.'; display: inline-block; padding: 1px 10px; border-radius: 3px;"';

        switch ($we_lang) {
            case 'en':
            // Dịch giá trị
                $we_prop_status_val = str_replace('Đã thuê', 'Rented out', $we_prop_status_val);
                $we_prop_status_val = str_replace('Còn phòng', 'Rentable', $we_prop_status_val);
                $we_prop_status_val = str_replace('Liên hệ', 'Call', $we_prop_status_val);
                // Dịch label
                $we_prop_status_lb = str_replace('Trạng thái phòng', 'Room status', $we_prop_status_lb);
                break;
    ...
    $we_prop_status_html = '<div '.$to_style.'>' . $we_prop_status_val . '</div>';
    $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_status_lb).':</strong> ' .$we_prop_status_html. '</div>';

    // Giặt sấy
    $we_prop_washing_machine = get_post_meta($post_id, 'we_prop_feature_18', true) ;
    $we_prop_washing_val = 'Liên hệ';
    $we_prop_washing_lb = 'Giặt sấy';
        if($we_prop_washing_machine!=''){
            $we_prop_washing_val = strval($we_prop_washing_machine);
        }
        
        switch ($we_lang) {
            case 'en':
                // Dịch giá trị
                $we_prop_washing_val = str_replace('Có', 'Yes', $we_prop_washing_val);
                $we_prop_washing_val = str_replace('Không', 'Not', $we_prop_washing_val);
                $we_prop_washing_val = str_replace('Liên hệ', 'Call', $we_prop_washing_val);
                // Dịch label
                $we_prop_washing_lb = str_replace('Giặt sấy', 'Washing machine', $we_prop_washing_lb);
                break;
    ...
    if(!wp_is_mobile()):
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_washing_lb).':</strong> ' .$we_prop_washing_val. '</div>';
    endif;

    // Wifi
    $we_prop_wifi = get_post_meta($post_id, 'we_prop_feature_19', true) ;
    $we_prop_wifi_val = 'Liên hệ';
    $we_prop_wifi_lb = 'Wifi';
        if($we_prop_wifi!=''){
            $we_prop_wifi_val = strval($we_prop_wifi);
        }
        
        switch ($we_lang) {
            case 'en':
                // Dịch giá trị
                $we_prop_wifi_val = str_replace('Có', 'Yes', $we_prop_wifi_val);
                $we_prop_wifi_val = str_replace('Không', 'Not', $we_prop_wifi_val);
                $we_prop_wifi_val = str_replace('Liên hệ', 'Call', $we_prop_wifi_val);
                // Dịch label
                $we_prop_wifi_lb = str_replace('Wifi', 'Wifi', $we_prop_wifi_lb);
                break;
    ...
    if(!wp_is_mobile()):
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_wifi_lb).':</strong> ' .$we_prop_wifi_val. '</div>';
    endif;

    // Chỗ để xe
    $we_prop_parking_lot = get_post_meta($post_id, 'we_prop_feature_20', true) ;
    $we_prop_parking_lot_val = 'Liên hệ';
    $we_prop_parking_lot_lb = 'Chỗ để xe';
        if($we_prop_parking_lot!=''){
            $we_prop_parking_lot_val = strval($we_prop_parking_lot);
        }
        
        switch ($we_lang) {
            case 'en':
                // Dịch giá trị
                $we_prop_parking_lot_val = str_replace('Có', 'Yes', $we_prop_parking_lot_val);
                $we_prop_parking_lot_val = str_replace('Không', 'Not', $we_prop_parking_lot_val);
                $we_prop_parking_lot_val = str_replace('Liên hệ', 'Call', $we_prop_parking_lot_val);
                // Dịch label
                $we_prop_parking_lot_lb = str_replace('Chỗ để xe', 'Parking lot', $we_prop_parking_lot_lb);
                break;
    ...
    if(!wp_is_mobile()):
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'"><strong>'.trim($we_prop_parking_lot_lb).':</strong> ' .$we_prop_parking_lot_val. '</div>';
    endif;
    ...
    * Tiện ích
    */
    $we_feature_lb = 'Tiện ích';
    switch ($we_lang) {
        case 'en':
            // Dịch label
            $we_feature_lb = str_replace('Tiện ích', 'Features', $we_feature_lb);
            break;
        
        default:
            break;
        }
    $return_string.= '<div class="listing_detail col-md-12"><strong><h4 style="color: #FF7925!important;">'.$we_feature_lb.'</h4></strong></div>';
    ...
    $we_feature_arr = [
        ...
        '3' => [
            'viLabel' => 'Set tủ bếp',
            'enLabel' => 'Set of kitchen cabinets',
            'icon'    => '<i class="fas fa-cookie-bite"></i>',
        ],
        '4' => [
            'viLabel' => 'Set bàn làm việc',
            'enLabel' => 'Set of desks',
            'icon'    => '<i class="fas fa-chair"></i>',
        ],
        '5' => [
            'viLabel' => 'Nệm ga gối',
            'enLabel' => 'Mattresses and pillows',
        ...
        '8' => [
            'viLabel' => 'Tranh ảnh decor',
            'enLabel' => 'Decorative pictures',
    ...
    foreach($we_feature_arr as $key_arr => $item_arr)
    {
        $we_index = 'we_prop_feature_'.($key_arr + 1);
        $we_prop_feature = get_post_meta($post_id, $we_index, true) ;
        $we_prop_feature_val = 'Liên hệ';
        $we_prop_feature_lb = $item_arr['viLabel'];
        $we_prop_feature_icon = !empty($item_arr['icon']) ? $item_arr['icon'] : '<i class="fas fa-gem"></i>';
        if($we_prop_feature!=''){
            $we_prop_feature_val = strval($we_prop_feature);
        }

        switch ($we_lang) {
            case 'en':
                // Dịch giá trị
                $we_prop_feature_val = str_replace('Có', 'Yes', $we_prop_feature_val);
                $we_prop_feature_val = str_replace('Không', 'Not', $we_prop_feature_val);
                $we_prop_feature_val = str_replace('Liên hệ', 'Call', $we_prop_feature_val);
                // Dịch label
                $we_prop_feature_lb = str_replace($item_arr['viLabel'], $item_arr['enLabel'], $we_prop_feature_lb);
                break;
            
            default:
                break;
        }
        $return_string.= '<div class="listing_detail col-md-'.$colmd.'">'.$we_prop_feature_icon.' <strong>'.trim($we_prop_feature_lb).':</strong> ' .$we_prop_feature_val. '</div>';
    }

    // End Alan Hong show custmo fields
        ...
        foreach($wpestate_submission_page_fields_array as $key=>$value):
            ...
            if(strtolower($energy_class)==strtolower($value) ){
                if($type=='energy'){
                    ...
                    '.$energy_index.' kWh/m²a '.esc_html__('Energy class ','wpresidence').' '.esc_html($energy_class).'</div>';
                }else{
                    $grades.='<div class="indicator-energy" data-energyclass="'.esc_attr($value).'">
                    '.$energy_index.' kgCO2/m²a '.esc_html__('Greenhouse gas emissions class  ','wpresidence').' '.esc_html($energy_class).'</div>';
