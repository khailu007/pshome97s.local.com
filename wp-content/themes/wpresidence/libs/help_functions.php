...
if (!function_exists('wpestate_return_property_status')):
    ...
    function wpestate_return_property_status($post_id, $return_type = '') {
        $to_return = 'Liên hệ';
        $property_status = get_post_meta($post_id, 'we_prop_status', true) ;
        if($property_status!=''){
            $to_return = strval($property_status);
            $to_return = ($to_return == 'Không có sẵn') ? 'Liên hệ' : $to_return;
        }
        ...
        $to_style = 'style="background: '.$to_background.'"';
        // Begin Alan Hong break after get property status
        $we_lang = ICL_LANGUAGE_CODE;

        switch ($we_lang) {
            case 'en':
                $to_return = str_replace('Còn phòng', 'Rentable', $to_return);
                $to_return = str_replace('Đã thuê', 'Rented out', $to_return);
                $to_return = str_replace('Liên hệ', 'Call', $to_return);
                $to_return = str_replace('Không có sẵn', 'Not Available', $to_return);
                break;
            
            default:
                break;
        }
        // End Alan Hong break after get property status
        $to_return = '<div class="ribbon-inside" '.$to_style.'>' . $to_return . '</div>';
...
if (!function_exists('wpestate_show_price')):
    ...
    function wpestate_show_price($post_id, $wpestate_currency, $where_currency, $return = 0,$second="no") {
        ...
        // Begin Alan Hong change / month
        $price_label = '/ tháng';
        $we_lang = ICL_LANGUAGE_CODE;
        switch ($we_lang) {
            case 'en':
                $price_label = str_replace('tháng', 'month', $price_label);
                break;
            
            default:
                break;
        }
        // End Alan Hong change / month
...
if (!function_exists('wpestate_build_dropdown_adv')):
    ...
    function wpestate_build_dropdown_adv($appendix, $ul_id, $toogle_id, $values, $values1, $get_var, $select_list, $active = '') {
        ...
        switch ($we_lang) {
            case 'en':
                $return_string = str_replace('Số người', 'No.People', $return_string);
                $return_string = str_replace('Nuôi pet', 'Pet', $return_string);
                $return_string = str_replace('Trạng thái phòng', 'Status', $return_string);
                $return_string = str_replace('Đã Thuê', 'Rented out', $return_string);
                $return_string = str_replace('Còn Phòng', 'Rentable', $return_string);
                $return_string = str_replace('Có', 'Yes', $return_string);
                $return_string = str_replace('Không', 'No', $return_string);
                break;
        ...
        if ($search_field == 'none') {
            ...
            if (isset($_GET['advanced_city']) && trim($_GET['advanced_city']) != '' && $_GET['advanced_city'] != 'all') {
                ...
                if ($label == '') {
                    ...
                    $advanced_city_value = esc_html__($wardLabel, 'wpresidence');
                }
                $advanced_city_value1 = 'all';
            }
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'adv-search-city', 'advanced_city', $advanced_city_value, $advanced_city_value1, 'advanced_city', $select_city_list);
            ...
            if (isset($_GET['advanced_contystate']) && trim($_GET['advanced_contystate']) != '' && $_GET['advanced_contystate'] != 'all') {
                ...
                if ($label == '') {
                    ...
                    $advanced_county_value = esc_html__($districtLabel, 'wpresidence');
                }
                $advanced_county_value1 = 'all';
            }
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'adv-search-countystate', 'county-state', $advanced_county_value, $advanced_county_value1, 'advanced_contystate', $select_county_state_list);
                    ...
                    while ($i < count($custom_fields)) {
                        ...
                        if (($slug_drop == $adv_search_what_key || $slug_drop == $search_field) && $custom_fields[$i][2] == 'dropdown') {
                            ...
                            if (function_exists('icl_translate')) {
                                ...
                                switch ($we_lang) {
                                    case 'en':
                                        $initial_key = ($initial_key == 'Số người') ? 'No.People' : $initial_key;
                                        $initial_key = ($initial_key == 'Trạng thái phòng') ? 'Status' : $initial_key;
                                        $initial_key = ($initial_key == 'Nuôi pet') ? 'Pet' : $initial_key;
                                        break;
                            ...
                            foreach ($dropdown_values_array as $drop_key => $value_drop) {
                                ...
                                switch ($we_lang) {
                                    case 'en':
                                        $value_drop = ($value_drop == 'Có') ? 'Yes' : $value_drop;
                                        $value_drop = ($value_drop == 'Không') ? 'No' : $value_drop;
                                        $value_drop = ($value_drop == 'Đã thuê') ? 'Rented out' : $value_drop;
                                        $value_drop = ($value_drop == 'Còn phòng') ? 'Rentable' : $value_drop;
                                        break;
...
if (!function_exists('wpestate_get_city_select_list')):
    ...
    function wpestate_get_city_select_list($args) {
        ...
        if ($categ_select_list === false) {
            ...
            $label = wpestate_return_default_label($adv_search_what, $adv_search_label, 'cities', esc_html__($wardLabel, 'wpresidence'));
...
if (!function_exists('wpestate_get_county_state_select_list')):
    ...
    function wpestate_get_county_state_select_list($args) {
        ...
        if ($categ_select_list === false) {
            ...
            $label = wpestate_return_default_label($adv_search_what, $adv_search_label, 'county / state', esc_html__($districtLabel, 'wpresidence'));
...
if (!function_exists('wpestate_show_advanced_search_options_redux')):
    ...
    function wpestate_show_advanced_search_options_redux($adv_search_what_value) {
        ...
        $admin_submission_array = array('Location' => esc_html('Location', 'wpresidence'),
            ...
            // 'property_duykhai' => esc_html('Pet', 'wpresidence'),
