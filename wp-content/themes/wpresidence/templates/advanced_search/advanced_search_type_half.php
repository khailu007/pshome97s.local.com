...
<!--    <form role="search" method="get"   action="<?php print esc_url($adv_submit); ?>" >-->
        ...
        <div class="row">
            ...
            wp_enqueue_script('geolocation_search_code');
            }
            ?>
            ...
            if ( $adv_search_type==11 ){
             ...
             if ( $adv_search_type==6 ){
                ...
                foreach($adv_search_what as $key=>$search_field){
                    switch ($we_lang) {
                        case 'en':
                            $adv_search_label[$key] = ($adv_search_label[$key] == 'Quận') ? 'District' : $adv_search_label[$key];
                            $adv_search_label[$key] = ($adv_search_label[$key] == 'Phường') ? 'Ward' : $adv_search_label[$key];
                            break;
                        
                        default:
                            $adv_search_label[$key] = $adv_search_label[$key];
                            break;
                    }                 
                    wpestate_show_search_field($adv_search_label[$key],'half',$search_field,$action_select_list,$categ_select_list,$select_city_list,$select_area_list,$key,$select_county_state_list);
                }
                $we_refresh_slug = ($we_lang == 'en') ? 'search-pshome97s' : 'tim-kiem';
                $we_refresh_url = home_url( $we_refresh_slug ); 
                $we_refresh_btn = '<a class="wpresidence_button  search_button_use_hover_effect_no" href="'.$we_refresh_url.'" alt="'.$we_refresh_slug.'" style="color: #fff;">';
                $we_refresh_btn .= '<i class="fas fa-sync-alt"></i> ';
                $we_refresh_btn .= 'Refresh';
                $we_refresh_btn .= '</a>';
                print $we_refresh_btn;
            }
        }else{
        ?>
