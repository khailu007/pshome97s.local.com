...
if (!empty($cf7sr_key) && !empty($cf7sr_secret) && !is_admin()) {
    function enqueue_cf7sr_script() {
        ...
        // Change language
        $we_lang = 'vi';
        $we_lang = !empty(ICL_LANGUAGE_CODE) ? ICL_LANGUAGE_CODE : 'vi';
        $cf7sr_script_url = 'https://www.google.com/recaptcha/api.js?onload=cf7srLoadCallback&render=explicit&hl='.$we_lang;
        //
        $cf7sr_key = get_option( 'cf7sr_key' );
        ?>
